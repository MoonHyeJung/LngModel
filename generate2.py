...
def generate_with_temperature(model, seed_word, temperature, device, char_to_idx, idx_to_char, length=200):
    ...
    input_seq = torch.tensor([char_to_idx[c] for c in seed_word]).unsqueeze(0).to(device)  # 시드 단어를 인덱스로 변환하여 텐서로 만들고 GPU로 이동

    samples = seed_word  # 생성된 문자를 저장할 문자열 초기화
...
def main():
    ...
    temperatures = [0.5, 1.0, 1.5, 2.0]  # 다양한 온도 값 리스트

    # 시드 단어 목록
    seed_words = ['MARCIUS', 'BRUTUS', 'SICINIUS', 'CORIOLANUS', 'MENENIUS']
    ...
    with open(filename, 'w') as f:
        for temperature in temperatures:
            ...
            for seed_word in seed_words:
                generated_text = generate_with_temperature(model, seed_word, temperature, device, char_to_idx, idx_to_char, length)
                f.write(f"Seed word: {seed_word}\n")
                f.write(f"Generated text: {generated_text}\n\n")
                print(f"Temperature: {temperature}, Seed word: {seed_word}")
